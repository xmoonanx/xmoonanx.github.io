---
title: "SQL Injection: 발생 원리와 방어 방법 - Secure Coding 6-2"
description: "SQL Injection이 왜 발생하는지, 공격자가 어떤 방식으로 쿼리를 변조하는지, 그리고 prepared statement·입력 검증·권한 제어로 어떻게 방어하는지 정리합니다."
...
> SQL Injection은 사용자 입력이 쿼리 구조 자체를 바꿔버릴 때 발생하는 대표적인 웹 취약점이다. 이 글은 발생 원인, 공격 방식, 실습 관점의 확인 방법, 그리고 실제 방어 전략까지 데이터 계층 보안의 핵심 흐름으로 정리한다.

## What this post covers

- SQL Injection이 생기는 구조적 원인
- Error-based, Union-based, Blind SQL Injection의 차이
- 입력 검증, 권한 통제, 로그 모니터링 중심의 방어 전략
...
- SQL Injection은 단순한 문자열 문제라기보다, 사용자 입력이 쿼리 의미를 바꾸는 구조적 문제다.
- 공격자는 Error-based, Union-based, Blind 방식으로 서로 다른 경로에서 정보를 빼내거나 권한을 우회한다.
- 가장 현실적인 방어 축은 prepared statement 같은 안전한 쿼리 처리, 입력 검증, 최소 권한, 그리고 로그 기반 탐지 체계를 함께 두는 것이다.
