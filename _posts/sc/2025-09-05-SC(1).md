---
title: "시큐어 소프트웨어 개발의 기초 - Secure Coding 1"
description: "소프트웨어 보안이 무엇을 막으려는지, 왜 secure development가 중요한지, 그리고 이후 시큐어 코딩 주제가 어떤 기반 위에 놓이는지 정리합니다."
...
> 시큐어 코딩은 취약점 몇 개를 외우는 과목이 아니라, 개발 과정 전체에서 보안을 어떻게 기본값으로 넣을지 정하는 출발점이다. 이 글은 현대 애플리케이션 보안 과제와 SW개발보안, DevSecOps, Security by Design의 큰 그림을 먼저 잡는다.

## What this post covers

- 현대 애플리케이션이 반복해서 마주치는 보안 과제
- SW개발보안 프로세스와 DevSecOps의 역할
- 보안내재화, Threat Modeling, Zero Trust가 왜 이어지는지

국내 SW에서 발견되는 취약점들은 이미 널리 알려진 약점이 반복적으로 재등장하는 경우가 많다. 그래서 시큐어 코딩은 특정 사고를 예외로 보는 대신, 개발 과정 전체에서 보안을 기본값으로 다루는 태도에서 출발해야 한다.
...
## 사고사례를 통한 시큐어코딩 이해
---
📚**<span style="color: #008000">시큐어 소프트웨어</span>**: 소프트웨어에 실수나 오류로 인한 결함, 결점, 버그가 없어서 장애, 문제가 발생하지 않는 소프트웨어
...
- 시큐어 코딩의 출발점은 개별 취약점 대응보다, 개발 수명주기 전체에 보안을 넣는 구조를 이해하는 것이다.
- SW개발보안, DevSecOps, Security by Design, Threat Modeling은 서로 대체 관계가 아니라 연결된 보안 운영 방식이다.
- 이후 기준 문서, 위협 모델링, 인증, 입력 검증, SQL Injection 같은 개별 주제를 읽을 때 이 글이 전체 지도를 제공한다.
