---
title: "[OS] Operating System(3-1): Process Concept/Scheduling"
...
> stack, heap은 동적으로 공간을 할당, free하기 때문에 양쪽으로 늘어났다 줄었다 함 → 그래서 마주보게 디자인
...
📚 **Context Switch**: 실행 중인 `old process`의 **상태를 저장**하고, **저장된** `new process`를 불러온다
...
> P0에 `timer interrupt`가 걸리면, 현재 register와 IP값을 저장(user→kernel mode이기 때문에 IP도 저장)
> 스케쥴링으로 실행시간 계산(만약 시간이 남았으면 **바로 return해서 P0실행**) → 다 썼으면 P0의 PCB를 `Ready Queue`에 넣음, 다음에 실행할 P1을 `Ready Queue`에서 선택
> switching을 해서 **현재 register, IP값을 P0 PCB에 저장**, **P1 PCB에서 저장된 register, IP값을 복원**하자마자 실행(`Jump`: 직접 가지 못하기때문에 jump라함)
