---
title: "캐시와 메모리 계층 구조의 이해 - Computer Architecture 11-1"
description: "캐시가 왜 필요한지, 메모리 계층 구조가 어떤 착각을 만들어 성능을 높이는지, 그리고 locality와 direct mapped cache의 기초를 정리합니다."
...
> 캐시는 단순히 빠른 메모리가 아니라, 프로세서가 큰 메모리를 빠르게 쓰는 것처럼 느끼게 만드는 핵심 장치다. 이 글은 memory hierarchy, locality, cache의 기본 용어, direct mapped cache의 출발점을 한 흐름으로 정리한다.

## What this post covers

- 메모리 기술과 계층 구조의 큰 그림
- locality가 캐시 성능을 가능하게 하는 이유
- cache hit/miss, tag, valid bit, direct mapped cache의 기초
...
- 메모리 계층 구조는 속도와 용량 사이의 trade-off를 계층적으로 해결하는 방식이다.
- locality가 있기 때문에 작은 캐시로도 큰 성능 이득을 만들 수 있다.
- direct mapped cache를 이해하면 이후 direct mapped, set associative, placement 전략을 더 쉽게 따라갈 수 있다.
